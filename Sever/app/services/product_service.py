...
from app.models import (
    Product, Phone, Laptop, Tablet, Audio,
    ProductVariation, Category, Brand, Review
)
...
class ProductService:
    ...
    def create_product(product_data, image_files):
        ...
        try:
            ...
            product = ProductService._build_product_instance(
                ProductModel,
                product_data,
                image_urls
            )
            ...
            if product_data.get('hasVariation', False):
                ...
                if variations_json:
                    ProductService._create_variations(
                        product.id, variations_json)
    ...
    def _create_variations(product_id, variations_json):
        ...
        try:
            variations_list = json.loads(variations_json) if isinstance(
                variations_json, str) else variations_json
    ...
    def update_product(product_id, product_data, new_image_files=None):
        ...
        try:
            ...
            if product.hasVariation and 'variations' in product_data:
                ProductService._update_variations(
                    product.id, product_data['variations'])
    ...
    def _update_type_specific_fields(product, data):
        ...
        if isinstance(product, (Phone, Tablet)):
            for field in ['ram', 'storage', 'battery', 'main_camera', 'front_camera',
    ...
    def _update_variations(product_id, variations_data):
        ...
        try:
            variations_list = json.loads(variations_data) if isinstance(
                variations_data, str) else variations_data
    ...
    def delete_product(product_id):
        ...
        """
        try:
            from app.models import CartItem, OrderItem, WishList, CompareItem
