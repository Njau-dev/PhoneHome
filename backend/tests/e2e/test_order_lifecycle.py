...
class TestOrderLifecycle:
    ...
    def test_order_document_generation(self, client, auth_headers, order):
        ...
        invoice_response = client.get(
            f'/api/orders/document/{order.order_reference}/invoice',
            headers=auth_headers
        )
        assert invoice_response.status_code in [
            200, 400]  # 400 if not delivered yet

        # If order is delivered, should be able to get receipt
        if order.status == 'Delivered':
            receipt_response = client.get(
                f'/api/orders/document/{order.order_reference}/receipt',
                headers=auth_headers
            )
    ...
    def test_multiple_orders_different_users(
        self, client, create_user, product, multiple_products
    ):
        """
        ...
        for i in range(3):
            ...
            # Add to cart
            selected_product = multiple_products[i] if i < len(
                multiple_products) else product
            client.post(
                '/api/cart/',
                headers=headers,
