...
class TestAuthFailureScenarios:
    ...
    def test_expired_reset_token_rejected_and_token_not_cleared(self, client, db, user, monkeypatch):
        class _FakeEmailService:
            def send_password_reset(self, *_args, **_kwargs):
                return {'success': True}

        monkeypatch.setattr('app.api.auth.routes.EmailService.init_app', lambda _app: _FakeEmailService())
        ...
        reset_response = client.post(
            f'/api/auth/reset-password/{db_user.reset_token}',
    ...
    def test_reused_reset_token_fails_after_successful_reset(self, client, user, monkeypatch):
        ...
        first_reset = client.post(f'/api/auth/reset-password/{token}', json={'password': 'newPassword123'})
        assert first_reset.status_code == 200
        assert first_reset.json['success'] is True

        reused_reset = client.post(f'/api/auth/reset-password/{token}', json={'password': 'anotherPass123'})
...
class TestCheckoutAndOrderFailureScenarios:
    def test_empty_cart_checkout_fails_with_no_order_or_payment(self, client, db, auth_headers, user):
    ...
    def test_checkout_rejects_mismatched_total_and_preserves_cart(self, client, db, auth_headers, user, product):
        add_cart = client.post('/api/cart/', headers=auth_headers, json={'productId': product.id, 'quantity': 1})
    ...
    def test_canceled_order_path_blocks_further_payment_retry(self, client, db, auth_headers, admin_user, user, product):
        client.post('/api/cart/', headers=auth_headers, json={'productId': product.id, 'quantity': 1})
        ...
        order_response = client.post('/api/payments/mpesa/initiate', headers=auth_headers, json={
        ...
        retry_response = client.post('/api/payments/mpesa/retry', headers=auth_headers, json={
            'phone_number': '254712345678',
    ...
    def test_user_cannot_read_other_user_order(self, client, create_user, product):
        ...
        login_a = client.post('/api/auth/login', json={'email': user_a.email, 'password': 'password123'})
        login_b = client.post('/api/auth/login', json={'email': user_b.email, 'password': 'password123'})
        ...
        create_order = client.post('/api/orders/', headers=headers_a, json={
            'address': {
                'firstName': 'Alice',
                'lastName': 'A',
...
class TestPaymentLifecycleFailureScenarios:
    ...
    def test_failed_mpesa_result_code_updates_order_and_payment(self, client, db, user, product):
        ...
        callback = client.post('/api/payments/ganji/inaflow', json={
            'Body': {
                'stkCallback': {
    ...
    def test_payment_retry_security_and_retry_limit_by_state(self, client, db, create_user, product):
        ...
        payment = Payment(order_reference='PHK-901', amount=float(product.price), payment_method='MPESA', status='Failed')
        db.session.add(payment)
        db.session.flush()
        order = Order(
            user_id=user_a.id,
        ...
        forbidden_retry = client.post('/api/payments/mpesa/retry', headers=headers_b, json={
        ...
        blocked_retry = client.post('/api/payments/mpesa/retry', headers=headers_a, json={
            'phone_number': '254700000001',
            'order_reference': 'PHK-901'
        })
...
class TestCompareWishlistEdgeFlows:
    def test_compare_add_remove_same_product_repeatedly_and_isolated(self, client, db, create_user, product):
        ...
        for _ in range(2):
            add_response = client.post('/api/compare/', headers=headers_a, json={'product_id': product.id})
    ...
    def test_wishlist_add_remove_same_product_repeatedly_and_isolated(self, client, db, create_user, product):
        user_a = create_user(username='wish-a', email='wish-a@example.com', phone_number='0703000001')
        user_b = create_user(username='wish-b', email='wish-b@example.com', phone_number='0703000002')
        ...
        for _ in range(2):
            add_response = client.post('/api/wishlist/', headers=headers_a, json={'product_id': product.id})
            assert add_response.status_code in [200, 201]
            assert add_response.json['success'] is True
