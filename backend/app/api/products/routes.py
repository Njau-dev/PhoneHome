...
def get_products():
    ...
    try:
        ...
        return (
            jsonify(format_response(True, {"products": products}, "Products fetched successfully")),
            200,
        )
    except Exception as e:
        logger.error(f"Error fetching products: {str(e)}")
        return (
            jsonify(format_response(False, None, "An error occurred while fetching products")),
            500,
        )
...
def get_product(product_id):
    ...
    try:
        ...
        return (
            jsonify(format_response(True, {"product": product}, "Product fetched successfully")),
            200,
        )
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while fetching the product")),
            500,
        )
...
def create_product():
    ...
    try:
        ...
        required_fields = ["name", "price", "description", "category_id", "brand_id", "type"]
        for field in required_fields:
            if not request.form.get(field):
                return (
                    jsonify(format_response(False, None, f"Missing required field: {field}")),
                    400,
                )
        ...
        if not image_files:
            return (
                jsonify(format_response(False, None, "At least one product image is required")),
                400,
            )
        ...
        product, error = ProductService.create_product(product_data, image_files)
        ...
        return (
            jsonify(
                format_response(
                    ...
                    f"{product_type.capitalize()} added successfully!",
                )
            ),
            201,
        )
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while creating the product")),
            500,
        )
...
def update_product(product_id):
    ...
    try:
        ...
        if "hasVariation" in request.form:
            product_data["hasVariation"] = (
                request.form.get("hasVariation", "false").lower() == "true"
            )
        if "isBestSeller" in request.form:
            product_data["isBestSeller"] = (
                request.form.get("isBestSeller", "false").lower() == "true"
            )
        ...
        product, error = ProductService.update_product(
            product_id, product_data, new_image_files if new_image_files else None
        )

        if error:
            return jsonify(format_response(False, None, error)), (
                400 if "not found" not in error.lower() else 404
            )
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while updating the product")),
            500,
        )
...
def delete_product(product_id):
    ...
    try:
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while deleting the product")),
            500,
        )
...
def get_products_by_type(product_type):
    ...
    try:
            ...
            return (
                jsonify(
                    format_response(
                        False,
                        None,
                        f"Invalid product type. Must be one of: {', '.join(valid_types)}",
                    )
                ),
                400,
            )
        ...
        return (
            jsonify(
                format_response(
                    True, {"products": filtered_products}, "Products fetched successfully"
                )
            ),
            200,
        )
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while retrieving products")),
            500,
        )
...
def get_products_by_category(category_id):
    ...
    try:
        ...
        products = ProductService.get_all_products()
        filtered_products = [p for p in products if p.get("category") == category.name]
...
def add_product_variations(product_id):
    ...
    try:
        ...
        product_data = {"hasVariation": True, "variations": data["variations"]}

        product, error = ProductService.update_product(product_id, product_data)
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while adding variations")),
            500,
        )
