...
def get_cart():
    ...
    try:
        ...
        return (
            jsonify(format_response(True, {"cart": cart_contents}, "Cart fetched successfully")),
            200,
        )
...
def add_to_cart():
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(format_response(False, None, "Invalid productId. Must be an integer")),
                400,
            )
        ...
        if not isinstance(data["quantity"], int) or data["quantity"] <= 0:
            return (
                jsonify(
                    format_response(False, None, "Invalid quantity. Must be a positive integer")
                ),
                400,
            )
        ...
        if data.get("selectedVariation"):
            ...
            if not isinstance(variation, dict):
                return (
                    jsonify(
                        format_response(False, None, "Invalid selectedVariation. Must be an object")
                    ),
                    400,
                )
            ...
            if "ram" not in variation or "storage" not in variation or "price" not in variation:
                return (
                    jsonify(
                        format_response(
                            False, None, "Variation must include ram, storage, and price"
                        )
                    ),
                    400,
                )
        ...
        success, message = CartService.add_to_cart(
            current_user_id, product_id, data["quantity"], variation_name, variation_price
        )

        if not success:
            return jsonify(format_response(False, None, message)), 400

        logger.info(f"User {current_user_id} added product {product_id} to cart")
...
def update_cart_item():
    ...
    try:
        ...
        success, message = CartService.update_cart_item(
            current_user_id, product_id, data["quantity"], variation_name
        )
...
def remove_from_cart():
    ...
    try:
        ...
        success, message = CartService.remove_from_cart(current_user_id, product_id, variation_name)

        if not success:
            status_code = 404 if "not found" in message.lower() else 400
            return jsonify(format_response(False, None, message)), status_code

        logger.info(f"User {current_user_id} removed product {product_id} from cart")
        return jsonify(format_response(True, None, message)), 200

    except Exception as e:
        logger.error(f"Error removing from cart: {str(e)}")
        return (
            jsonify(format_response(False, None, "An error occurred while removing from cart")),
            500,
        )
...
def get_cart_total():
    ...
    try:
        ...
        return (
            jsonify(
                format_response(
                    True, {"total": total, "currency": "KES"}, "Cart total calculated successfully"
                )
            ),
            200,
        )
        ...
        return (
            jsonify(format_response(False, None, "An error occurred while calculating total")),
            500,
        )
